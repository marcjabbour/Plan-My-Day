...
# === ASYNC WRAPPER FOR CREW OPERATIONS ===
async def run_planner_crew(user_input):
    activity_planning_crew = create_planner_crew()
    result = await activity_planning_crew.kickoff_async(inputs={"topic": user_input})
    return result

def run_food_crew(selected_activities, preference=""):
    food_crew = create_food_crew()
    result = food_crew.kickoff(inputs={"selected_activities": selected_activities, "preference": preference})
    return result
...
if user_input:
    ...
    if intent == "plan_day":
        with st.spinner("Planning your day..."):
            # Run the async crew operation in a new event loop
            try:
                ...
                if loop.is_running():
                    ...
                    import nest_asyncio
                    nest_asyncio.apply()
                    result = asyncio.run(run_planner_crew(user_input))
                else:
                    result = asyncio.run(run_planner_crew(user_input))
            except RuntimeError:
...
if st.session_state.phase == "food":
    ...
    with st.spinner("Finding restaurants nearby..."):
        result = run_food_crew(st.session_state.selected_activities, "")
        raw_food = result.raw
        try:
...
if st.session_state.phase == "refine_food":
    ...
    with st.spinner("Refining suggestions..."):
        result = run_food_crew(st.session_state.selected_activities, preference)
